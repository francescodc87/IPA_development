...
# allsamp allsamp choose return value type
...
"IPA.sampler.Add.Iso.Bio.int_noPot" <- function(P, Add, Iso, Bio, Int,
    ...
    for (it in 1:no.its){
        ...
        for (thism in ordine){
            ...
            # counting adducts
            #used to: p.add <- pot.add - colSums(matrix(Add[sampcomp[ind.rem[[thism]]],], ncol=Nc))
            #and: pot.add <-apply(Add[sampcomp,],2,sum)
            p.add <- colSums(matrix(Add[sampcomp[-ind.rem[[thism]]],], ncol=Nc))
            
            # counting isotopes
            # intensity ratios are applied to siege
