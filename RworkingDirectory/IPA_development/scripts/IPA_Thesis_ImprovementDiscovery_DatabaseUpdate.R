...
# save(massMZ,massRT,massInt, file="C:/Users/oyyqwhuiss/Desktop/peakInfo.Rdata")
load(file="C:/Users/oyyqwhuiss/Desktop/peakInfo.Rdata")
...
if (length(maxExprId[[1]]) != 0){
  ...
  MATCH (n:Experiment)
  WHERE n.id = {maxId}
  RETURN n.ppmNew as initMA, n.ppmAll as storedMA, n.ppmMean as maDistrMean, n.ppmStd as maDistrStd
  "
...
unknownP:{u}, limit:{limit}, rtWin:{rtWin}, it:{it}, burn:{burn}, delAdd:{delAdd}, delIso:{delIso}, delBio:{delBio}, ratioToll:{ratioToll}, 
...
cypher(graph, query, id = expr_maxId + 1, ionMode = ionMode, ppmAll = recMA, ppmInit = ma, ppmNew = maNew, ppmMean = ppmMean, ppmStd = ppmStd, 
       u = u, limit = limit, rtWin = rtWin, it = it, burn = burn, delAdd = delAdd, delIso = delIso, delBio = delBio, ratioToll = ratioToll,
       s_pkAdd = s_pkAdd, s_pkComp = s_pkComp, s_ma = s_ma, s_iso = s_iso, w_int = w_int, w_ma = w_ma, w_addLink= w_addLink, w_isoLink = w_isoLink,
