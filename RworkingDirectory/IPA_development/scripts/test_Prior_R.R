"test_R_IPA.massbased.prior" <- function(mass, compounds.mass, ppm, pr.limit =1e-02, unknown.prob=0.05,
  ...
  for(i in 1:M){
    ...
    testresult <- exp((-0.5*precision[i])*((compounds.mass-mass[i])^2))
    ...
    if(v){
      if(i %% it==0) {
        ...
        cat("Computing Prior in R, ",paste0((i*100)/M, "%", "\n"))
      }
    } 
